    ...
    try:
        ...
        if commit_type == "Bug":
            ...
            for key, value in added_calls.items():
                for item in calls_map[key]:
                    vertex_changes_map[item]['psd'] += 1
                    if not vertex_changes_map[item]['psd_date']:
                        vertex_changes_map[item]['psd_date'] = commit_date
                for item in get_callers(key, calls_map):
                    vertex_changes_map[item]['pds'] += 1
                    if not vertex_changes_map[item]['pds_date']:
                        vertex_changes_map[item]['pds_date'] = commit_date
